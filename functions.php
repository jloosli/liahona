...
add_action( 'wp_enqueue_scripts', function () {
	wp_enqueue_script( 'custom-scripts', get_stylesheet_directory_uri() . '/js/script.js', ['jquery'], null, true );
} );
...
function liahona_callout( $args = [], $content = null ) {
...
function liahona_featured_save( $post_id ) {
	...
	if ( ! wp_verify_nonce( $_POST['liahona_featured_noncename'], __FILE__ ) ) {
		return $post_id;
	}
	...
	if ( $current_data ) {
		if ( is_null( $new_data ) ) {
			delete_post_meta( $post_id, '_liahona_featured' );
		} else {
			update_post_meta( $post_id, '_liahona_featured', $new_data );
		}
...
.login h1 a { background: url(https://theliahonaproject.net/assets/facebook1.jpg) no-repeat center top;
 height: 175px;
 width: 100%;
 }
</style>';
